    ...
    <div class="content mx-4 sm:p-8">
        <form action="{{ route('report.store', ['menu' => $menu->id]) }}" method="POST">
            <div class="flex items-center justify-center mt-8">
                ...
                <input type="submit" onclick="reportComplete()" value="送信" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150" />
        </form>
    </div>
    </div>
